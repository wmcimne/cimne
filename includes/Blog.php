...
// class ET_Builder_Module_Blog extends ET_Builder_Module_Type_PostBased {
	...
	function init() {
		...
		//console_log( 'hello function init' );
		...
		if ( have_posts() ) {
			...
			<article id="post-<?php the_ID(); ?>" <?php post_class( 'et_pb_post clearfix' . $no_thumb_class . $overlay_class . $item_class ); ?>>
				...
				<?php } ?>
					...
					// Place here the CPT meta data
					// echo (get_post_meta($post->ID, 'type_of_event', true) != '') ? '<div class="type-of-event rectangle"><span class="rectangle-text">'. get_post_meta($post->ID, 'type_of_event', true) . '</span></div>' : "";	

					// echo ( get_the_category( $id )[0]->name  != '') ? '<div class="type-of-event rectangle"><span class="rectangle-text">'. get_the_category( $id )[0]->name . '</span></div>' : "";



					$id = $post->ID;
					$categoriesArray = get_the_category( $id );
					//$categories = get_the_category( $id )[0]->name;
					//$parent_cat =  get_category( get_the_category( $id )[0]->category_parent )->name;
					

					/**
					 * Displays event type categories
					 * 
					 * Iterates through an array of categories and outputs event type badges.
					 * For subcategories (parent != 0), checks if the parent category is 'events', 'eventos', or 'esdeveniments' and displays the subcategory name.
					 * For parent categories, displays the primary category of the post.
					 */
					if (!empty( $categoriesArray ) ) {
						...
						console_log("Categoria: " . esc_html( $category->name ) . "   ID -->  " . $id);
							if ( $category->parent != 0 ) {
								...
								if ( strtolower( $parent_cat ) === 'events' || strtolower( $parent_cat ) === 'eventos' || strtolower( $parent_cat ) === 'esdeveniments' ) {
									echo '<div class="type-of-event rectangle"><span class="rectangle-text">' . esc_html( $category->name ) . '</span></div>';
								}
								
							}else{
								//console_log("Categoria Padre: " . esc_html( $category->name ) . "   ID -->  " . $id);
								//echo '<div class="type-of-event rectangle"><span class="rectangle-text"> TEST TEST </span></div>';
								$categories = get_the_category( $id )[0]->name;
								if ( strtolower( $categories ) === 'events' || strtolower( $categories ) === 'eventos' || strtolower( $categories ) === 'esdeveniments' ) {
									echo '<div class="type-of-event rectangle"><span class="rectangle-text">' . esc_html( get_the_category( $id )[0]->name ) . '</span></div>';
								}
					...
					if (get_post_meta($post->ID, 'date', true) != '') { 
						...
						// Crear un objeto DateTime desde el string
						$evnt_date = DateTime::createFromFormat("Ymd", $date_str);
						$date_formateada_visible = $evnt_date->format("d M Y");
						$date_formateada_dato = strtotime($date_formateada_visible);
						

					} else {
						$date_formateada_visible = "";
						$date_formateada_dato = "";

					}
					if (get_post_meta($post->ID, 'start_date', true) != '') { 
					 
						$start_date_str = get_post_meta($post->ID, 'start_date', true);
						// Crear un objeto DateTime desde el string
						$start_date = DateTime::createFromFormat("Ymd", $start_date_str);
						$start_date_formateada_visible = $start_date->format("d M Y");
						$start_date_formateada_dato = strtotime($start_date_formateada_visible);
						

					} else {
						$start_date_formateada_visible = $date_formateada_visible;
						$start_date_formateada_dato = $date_formateada_dato;

					}
					if (get_post_meta($post->ID, 'finish_date', true) != '') { 
						$finish_date_str = get_post_meta($post->ID, 'finish_date', true);
						// Crear un objeto DateTime desde el string
						$finish_date = DateTime::createFromFormat("Ymd", $finish_date_str);
						$finish_date_formateada_visible = ' - '. $finish_date->format("d M Y");
						$finish_date_formateada_dato = strtotime($finish_date_formateada_visible);
						

					} else {
						$finish_date_formateada_visible = "";
						$finish_date_formateada_dato = "";

					}

					echo '<div class="date" date="'.$date_formateada_dato.'">' . $start_date_formateada_visible . $finish_date_formateada_visible . '</div>';

					// echo (get_post_meta($post->ID, 'date', true) != '') ? '<div class="date" date="'.get_post_meta($post->ID, 'date', true).'">'.get_post_meta($post->ID, 'date', true). '</div>' : "";

					if (get_post_meta($post->ID, 'attendance', true) != '') {
						$attendance_str = strtolower(trim(get_post_meta($post->ID, 'attendance', true)));
						// console_log("Attendance string: ".$attendance_str);
						$attendance_class = "";
						
						switch ($attendance_str) {
							case 'online':
								$attendance_class = "online";
								break;
							case 'hybrid':
								$attendance_class = "hybrid";
								break;
							case 'in person':
								$attendance_class = "in-person";
								break;
							default:
								$attendance_class = "";
						}
					...
					echo (get_post_meta($post->ID, 'attendance', true) != '') ? '<div class="attendance '.$attendance_class.'">'. get_post_meta($post->ID, 'attendance', true) . '</div>' : "";
